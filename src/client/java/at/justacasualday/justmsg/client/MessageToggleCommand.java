        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            ...
            dispatcher.register(ClientCommandManager.literal("msgt").executes(MessageToggleCommand::disable)
                    .then(ClientCommandManager.argument("target", StringArgumentType.string())
                            .suggests((context, builder) -> CommandSource.suggestMatching(PlayerManager.getAllOnlinePlayers(), builder))
                            .executes(context -> {
                                ...
                                if (!PlayerManager.getTargets().isEmpty()) {
                                    player.sendMessage(Text.literal("Currently Messaging: " + PlayerManager.getTargets().stream().collect(Collectors.joining(", ")) + "!"), false);
                                }
                                return 0;
                            })
                    ));
